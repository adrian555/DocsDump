...
ks init ${APP_DIR} --api-spec=version:v1.11.6
...
ks registry add pipeline github.com/adrian555/DocsDump/tree/master/pipeline
ks pkg install pipeline/pipeline
...
if [ "$WITH_KUBEFLOW" = true ]; then
  ...
  (cd ${KF_DIR} && mv /pipeline/kubeflow.tar.gz .)
